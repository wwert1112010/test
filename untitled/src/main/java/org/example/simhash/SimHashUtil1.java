...
public class SimHashUtil1 {
    ...
    public SimHashUtil1(String topicName, Integer hashCount) {
    ...
    private BigInteger simHash() {
        ...
        // 对内容进行分词处理
        List<String> terms = divideWord(this.topicName);
    ...
    private BigInteger getWordHash(String word) {
        ...
        if (StringUtils.isEmpty(word)) {
            ...
            while (word.length() < SimHashUtil1.WORD_MIN_LENGTH) {
     ...
     * @author huazai
     */
    public Double getSimilar(SimHashUtil1 simHashUtil) {
        ...
        // 处理海明具体百分比
        Double myScale = getMyScale(scale);

        Double formatScale = Double.parseDouble(String.format("%.2f", myScale));
     ...
     * @author huazai
     */
    private int getHammingDistance(SimHashUtil1 simHashUtil) {
    ...
    private Double getMyScale(Double scale) {
        if(scale >= 90D) return scale;
        BigDecimal bigDecimal = new BigDecimal(scale);
        BigDecimal temp1 = bigDecimal.multiply(bigDecimal).multiply(new BigDecimal("0.019"));
        double v = temp1.doubleValue();
        if(v<=60D){
            return 0.1*v;
        }
        return v;
